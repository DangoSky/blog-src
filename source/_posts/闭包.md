...
&emsp;&emsp;我对闭包的理解是，**在一个函数中使用了一个内部函数，而且在这个内部函数中使用到了外部函数的变量**。在一般情况下，函数一旦执行完毕其内部的变量就会被销毁无法再被访问到，但通过返回一个函数可以使得该函数作用域链上可见的变量在函数外依旧可见，而我们只要在函数外使用一个变量来接受该返回结果就可以延长它们的生命周期了。这也就是闭包的两个作用： 
